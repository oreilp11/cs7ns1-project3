...
def find_second_shortest_path(positions_list, start_node, end_node, broken_devices=None):
    if broken_devices is None:
        broken_devices = set()
    else:
        broken_devices = set(broken_devices)
    ...
    second_shortest_path = paths[1]
    print(f"Second shortest distance: {second_shortest_path[0]:0.3f} km")
    print("Path:", " -> ".join(second_shortest_path[1]))
    return [int(node) for node in second_shortest_path[1]], second_shortest_path[0]
